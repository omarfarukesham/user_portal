...
import { useEditPortal } from '@/services/portals/usePortals';
import { useNavigate } from 'react-router-dom';
import { toast } from 'react-toastify';

const PortalEditForm = ({ defaultData }) => {
  const { isLoading, mutate } = useEditPortal();
  ...
  return (
    <PageInnerContainer className='p-4'>
      <h2 className='text-2xl mb-5'>
        Edit <span className='font-bold'>{defaultData?.displayName}</span>
      </h2>
          ...
          <div className='grid grid-cols-2 gap-5'>
            ...
            <FormInput
              ...
              validations={{
                valueAsNumber: true,
                required:
                  'Please give a position (this is for showing them in a certain order in the portal)',
              }}
            />
            <FormInput
              name='path'
